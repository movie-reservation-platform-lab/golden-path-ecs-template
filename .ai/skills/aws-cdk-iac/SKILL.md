---
name: aws-cdk-iac
description: Use when designing, implementing, reviewing, or explaining AWS CDK infrastructure in ecs-infra, including construct choices, CloudFormation output, networking, IAM, ECS/Fargate, asset publishing, tests, synth/deploy workflow, and CDK best practices.
---

# AWS CDK IaC

Use this skill for AWS CDK work in `ecs-infra/`.
...
This repository uses an npm workspace CDK app:
...
Run CDK commands from the repository root through npm workspace scripts:
...
Do not assume a global `cdk` binary, and do not assume the CDK app lives at the
repository root.
...
For this repository, the service Dockerfile expects the repository root as the
