...
For this repo, the service image build context is the repository root because
`movie-reservation-service/Dockerfile` copies root workspace files.

Exclude unrelated folders such as generated output, docs, local AI tool state,
and unrelated workspaces when they are not needed for the Docker build.
...
```bash
npm -w ecs-infra run cdk -- diff -c allowedIngressCidr=203.0.113.10/32
```
